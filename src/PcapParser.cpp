...
void PcapParser::applySimba(const Network::UDP &udp)
{
    ...
    if (mdHeader->incremental())
    {
        ...
        case Simba::OrderUpdate::MSG_ID: {
            const auto update = Simba::cast<Simba::OrderUpdate>(udp.data() + offset, udp.dataSize() - offset);
            output = update->format();
            break;
        }
        case Simba::OrderExecution::MSG_ID: {
            const auto execution = Simba::cast<Simba::OrderExecution>(udp.data() + offset, udp.dataSize() - offset);
    ...
    else
    {
        ...
        case Simba::OrderBookSnapshot::MSG_ID:
            const auto snapshot = Simba::cast<Simba::OrderBookSnapshot>(udp.data() + offset, udp.dataSize() - offset);
